...
export function getSchemaForVersion(version: string): any {
  if (!schemaModules[version]) {
    ...
    throw new Error(
      `Schema version "${version}" is not available. Available versions: ${availableVersions.join(", ")}. Please update the "oiml.schemaVersion" setting to one of the available versions.`
    );
  }
