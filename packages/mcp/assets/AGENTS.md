...
This document serves as the complete reference for implementing OIML-based code generation. When given an intent file (`intent.yaml`), use this guide to perform actual code changes.
...
- **Standardized Schema**: All intents follow a consistent structure defined in the OIML schema
...
└── intents/                  # All intents organized by ticket/issue ID
    ...
    │   ├── plan.yaml         # Agent's planned changes (created before execution)
...
- `plan.yaml` - Implementation plan showing intended changes (created before code execution)
...
OIML supports the following intent types. For complete schema definitions, see [`@oiml/schema`](https://www.npmjs.com/package/@oiml/schema).
...
- **CRITICAL:** Use OIML MCP server: `mcp_oiml_validate_intent(filePath)` if available
...
- **CRITICAL:** Read `.oiml/project.yaml` - this file is required for all implementations
...
### 5. Create and Validate Implementation Plan

Before executing any code changes, create a `plan.yaml` file documenting your intended changes:

1. **Create plan.yaml** in the intent folder:

   ```typescript
   const planPath = ".oiml/intents/FEAT-123/plan.yaml";
   ...
   const plan = {
     ...
     intents_to_process: irObjects.length,
     template_used: {
       framework: template.framework,
       category: template.category,
       pack: template.template_pack,
...
2. **Build plan steps** from IR objects:

   For each IR object, create an intent step describing the changes:

   ```typescript
   for (const ir of irObjects) {
     const step = {
       kind: ir.kind, // e.g., "add_entity"
       scope: "data", // or "api", "ui", "capability"
       target: ir.entity?.name || ir.endpoint?.path || ir.component?.name,
       description: `Create ${ir.entity?.name} entity...`,
       changes: []
     };

     // Add planned changes for this intent
     if (ir.kind === "AddEntity") {
       step.changes.push({
         file: "prisma/schema.prisma",
         action: "modify",
         description: `Add ${ir.entity.name} model with fields...`,
         steps: ["Add model definition", "Add fields with types", "Add relationships if any"]
       });

       step.changes.push({
         file: "prisma/migrations/YYYYMMDDHHMMSS_add_entity/migration.sql",
         action: "create",
     ...
     plan.planned_changes.push(...step.changes);
   }
   ```

3. **Assess risk** based on planned changes:

   ```typescript
   // Analyze planned changes for risk factors
   const deletions = plan.planned_changes.filter(c => c.action === "delete");
   const schemaChanges = plan.planned_changes.filter(c => c.file.includes("schema.") || c.file.includes("migration"));

   if (deletions.length > 0 || schemaChanges.length > 5) {
     plan.risk_assessment.level = "high";
     plan.risk_assessment.factors.push("Database schema modifications");
     plan.risk_assessment.mitigations.push("Create database backup before migration");
   } else if (schemaChanges.length > 0) {
     plan.risk_assessment.level = "medium";
     plan.risk_assessment.factors.push("Database schema changes");
   } else {
     plan.risk_assessment.level = "low";
     plan.risk_assessment.factors.push("No significant risk factors");
   }
   ```

4. **Add warnings and prerequisites**:

   ```typescript
   // Add relevant warnings
   if (projectConfig.database?.autorun_migrations === false) {
     plan.warnings.push("Migration files will be created but not applied");
     plan.warnings.push("Run migrations manually when ready");
   }

   // Add prerequisites
   plan.prerequisites.config_checks.push("Database connection string configured in .env");
   plan.prerequisites.required_files.push(projectConfig.database?.schema || "prisma/schema.prisma");
   plan.prerequisites.dependencies.push("@prisma/client", "prisma");
   ```
...
6. **Validate plan** using MCP server:

   ```typescript
   const planValidation = await mcp_oiml_validate_plan({
   ...
   if (!planValidation.valid) {
     ...
     planValidation.errors.forEach(err => console.error(`  - ${err}`));
     throw new Error("Invalid plan - cannot proceed with code generation");
   }
   ```

7. **Review plan** (optional but recommended):
   - Display plan summary to user
   - Allow user to review planned changes
   - Get approval before proceeding (if required)

**Why Create a Plan?**

- **Transparency**: Users can see what will change before execution
- **Validation**: Ensures plan is complete and well-structured
- **Auditing**: Provides trail of what was planned vs. what happened
- **Risk Assessment**: Identifies potential issues before changes
- **Review**: Allows for human approval of high-risk changes

### 6. Process Each Intent IR

**After plan validation succeeds**, proceed with code generation:
...
If available, use the OIML MCP server for validation and transformation:

```javascript
// 1. Validate intent file
const validation = await mcp_oiml_validate_intent({
...
const plan = buildPlanFromIR(irResult.ir, templateInfo);
fs.writeFileSync(".oiml/intents/FEAT-123/plan.yaml", YAML.stringify(plan));

// 4. Validate plan
const planValidation = await mcp_oiml_validate_plan({
  content: fs.readFileSync(".oiml/intents/FEAT-123/plan.yaml", "utf8"),
  format: "yaml"
});
...
// 5. Execute code generation using IR
...
3. **Create implementation plan**: Always create `plan.yaml` before code execution
   - Document all planned changes
   - Assess risk factors
   - Validate with `mcp_oiml_validate_plan`
   - **Never execute code without a valid plan**
4. **Use IR for code generation**: Never use the raw intent.yaml for code generation - always use the transformed IR
5. **Follow conventions**: Match existing code patterns in the project
6. **Type safety**: Always use TypeScript types, never `any`
7. **Error handling**: Include comprehensive error handling
8. **Code organization**: Respect project.yaml paths and structure
9. **Migration safety**: Migrations are always created, but only automatically applied if `database.autorun_migrations: true`:
...
11. **Testing**: Generate test-friendly code structures
12. **Audit trail**: Maintain complete audit trail with `intent.yaml` → `plan.yaml` → `summary.yaml`
...
5. ✅ **Create and validate plan** (`plan.yaml`):
   - Build plan from IR objects
   - Document all planned changes
   - Assess risk factors
   - Validate plan using `mcp_oiml_validate_plan`
   - **STOP if plan is invalid**
6. ✅ **Process** `add_entity` IR:
...
1. **Location**: `.oiml/intents/{INTENT-ID}/summary.yaml`
...
4. **YAML Formatting Requirements**:
   ...
   **Best Practice**: When in doubt, quote strings in the `description` and `notes` fields, especially if they contain:
   ...
   - Technical descriptions with colons, pipes, or other YAML syntax characters

5. **Template Resolution**: Use the MCP `resolve_template` tool to determine which template pack to use:
...
The `plan.yaml` and `summary.yaml` files serve different purposes in the OIML workflow:

| Aspect         | plan.yaml                               | summary.yaml                         |
| -------------- | --------------------------------------- | ------------------------------------ |
| **Timing**     | Created **before** execution            | Created **after** execution          |
| **Purpose**    | Show intended changes                   | Document actual changes              |
| **Content**    | Planned changes, risk assessment        | Actual changes, errors encountered   |
| **Status**     | N/A (not yet executed)                  | `success`, `partial`, or `failed`    |
| **Validation** | Validated with `mcp_oiml_validate_plan` | No validation needed                 |
| **Changes**    | `planned_changes[]`                     | `changes[]` (what actually happened) |
| **Risk**       | `risk_assessment` (anticipated)         | N/A (already executed)               |
| **Errors**     | `warnings[]` (potential issues)         | `errors[]` (actual errors)           |
| **Use Case**   | Review, approval, transparency          | Auditing, debugging, history         |

**Workflow**:

1. Read `intent.yaml` (what to build)
2. Create `plan.yaml` (how we'll build it)
3. Validate plan and get approval
4. Execute code changes
5. Create `summary.yaml` (what we actually built)

**Why Both?**

- `plan.yaml`: Shows **intent** - what the agent plans to do
- `summary.yaml`: Shows **reality** - what actually happened
- Comparing them reveals deviations, errors, or unexpected changes
