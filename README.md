...
OIML (Open Intent Modeling Language) is an open source standard for AI-driven development that enables:

- **Declarative Development**: Code is generated by declaring intents and providing a single file as context to coding agents/IDEs
...
- **Templates**: Out of the box context tailored to specific frameworks, provided to agents during execution
...
- **Audit Trail**: Commit intent files and AI-executed steps to source control, offering a complete paper trail of generated code

## Background and Vision

Code generation tools, IDEs, and LLMs have reached an impressive level, but still yield inconsistent results given the same prompt, and often require extensive additional series of prompting and iterations to get code to its desired state. The resulting code, while it may work, can easily become spaghetti or difficult review if guardrails aren't put in place. 

What if you could provide structured data expressing what you want to build, and AI coded it perfectly?

It may not be possible yet, but in order to work toward this state, there needs be a global standard that companies, projects, and solo developers conform to that enables AI (and heck, even real people) to generate code in a consistent, auditable way and that also fits within the frameworks, styles, and standards of a given code base. The mission of OIML is to become that standard.

OIML aims to be framework and LLM agnostic, supporting a wide range of popular stacks (database / API / UI), and compatible across all current and future LLMs (e.g. Sonnet-4.5, GPT-5, Gemini, Cursor Composer).

In a nutshell, OIML is a combination of intent schema, project schema, and stack-specific context that is all fed through an LLM, encouraging it to produce the same or similar results on each run.

For obvious reasons, OIML must remain open source and never controlled by one company, while allowing others to benefit personally or commerically.
...
│   ├── INT-1.oiml.yaml       # Unique identifier of intent, or JIRA/Linear ticket # (recommended)
│   ├── INT-2.oiml.yaml
│   └── INT-3.oiml.yaml
└── output/                   # Stored summary of what AI did
    ├── INT-1.oiml.yaml
    ├── INT-2.oiml.yaml
    └── INT-3.oiml.yaml
```
...
The following example illustrates the power of OIML. Given a single intent file (YAML) with a total of 9 intents (3 database entities + 6 APIs), Claude Sonnet-4.5 was able to successfully create an entire functional backend from scratch, with the YAML file as its only context, in appx. 2 minutes using Prisma + Next.js. This was tested with both Cursor and Claude Code yielding similar restuls. The summary file of generated code changes, which is a key component of the OIML audit trail, is included below.

Click the below thumbnail to view the video on YouTube:

<a href="https://www.youtube.com/watch?v=RRXTZsEWEiA" target="_blank">
  <img src="assets/thumbnail.png" alt="Video">
</a>
...
Below is an example context snippet provided to LLMs during execution.
...
1. **UI Intents:** intents to create new components, simple layouts, detail views, new routes / route components, CRUD tables, forms
2. **Adapters:** intents to handle common application tasks such as user authentication, file upload, web sockets, and streaming data
3. **Integrations:** intents to integrate commonly used third party services (e.g. Stripe, SendGrid, OpenAI, Anthropic)
4. Additional templates and examples

## Contributions

Want to contribute? Send an email to [admin@oiml.dev](mailto:admin@oiml.dev).