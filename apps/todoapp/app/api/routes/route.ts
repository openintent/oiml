      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          const methods = ['GET', 'POST', 'PATCH', 'PUT', 'DELETE'].filter(
            (method) => routeContent.includes(`export async function ${method}`)
          );
          
          // Create endpoint for each method
          for (const method of methods) {
            let description = `${method} ${routePath.replace(/\\/g, '/').replace(/route\.ts$/, '').replace(/\[.*?\]/g, ':id')}`;
            ...
            if (description === method) {
              ...
              const action = method === 'GET' ? 'Fetch' : method === 'POST' ? 'Create' : method === 'PATCH' ? 'Update' : method === 'DELETE' ? 'Delete' : 'Handle';
