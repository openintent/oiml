    ...
    waitForPortAvailable(NEXT_PORT, 3000).then(() => {
      setTimeout(() => {
        ...
        if (isDev) {
          ...
          if (!serverFile) {
            const searchedPaths = [...possiblePaths, "(recursive search in standalone directory)"];
...
ipcMain.handle("watch-folder", async (_event: unknown, folderPath: string) => {
  try {
    ...
    const oimlPath = join(folderPath, ".oiml");
    if (!existsSync(oimlPath)) {
      return { success: true }; // No error, just nothing to watch
    }

    // Watch the entire .oiml directory to catch all changes
    // This ensures we catch changes to project.yaml and all intent.yaml files
    fileWatcher = chokidar.watch(oimlPath, {
      ignored: (path: string) => {
        // Always watch YAML files
        if (path.endsWith(".yaml") || path.endsWith(".yml")) {
          return false;
        }
        // For directories, allow them so we can watch files inside
        // Only ignore if it's a dotfile that's not .oiml itself
        const pathParts = path.split(/[/\\]/);
        const lastPart = pathParts[pathParts.length - 1];
        // Ignore dotfiles/dot-directories except .oiml itself
        if (lastPart.startsWith(".") && lastPart !== ".oiml") {
          return true;
        }
        return false;
      },
      ...
      binaryInterval: 500,
      depth: 10 // Watch deep enough to catch all intent.yaml files
    });

    // Helper function to send file-changed event
    const sendFileChangedEvent = (eventType: string, path: string) => {
      if (mainWindow && !mainWindow.isDestroyed()) {
        const eventData = {
          event: eventType,
          path,
    ...
    fileWatcher.on("change", (path: string) => {
      ...
      if (path.endsWith(".yaml") || path.endsWith(".yml")) {
        sendFileChangedEvent("change", path);
      }
    });
    ...
    fileWatcher.on("unlink", (path: string) => {
      if (path.endsWith(".yaml") || path.endsWith(".yml")) {
        sendFileChangedEvent("unlink", path);
      }
    });

    // Handle directory additions (new intent folder created)
    fileWatcher.on("addDir", (path: string) => {
      // When a new directory is added in the intents folder, reload
      if (path.startsWith(intentsPath) && path !== intentsPath) {
        sendFileChangedEvent("addDir", path);
      }
    });

    // Handle directory deletions (intent folder removed)
    fileWatcher.on("unlinkDir", (path: string) => {
      // When a directory is removed from the intents folder, reload
      if (path.startsWith(intentsPath) && path !== intentsPath) {
        sendFileChangedEvent("unlinkDir", path);
      }
    });

    fileWatcher.on("ready", () => {
      // Manual polling to detect directory structure changes
      // This helps catch cases where folders are added/deleted but events aren't fired
      let lastDirStructure: Set<string> = new Set();
      const testPolling = setInterval(() => {
        try {
          ...
          currentDirs.forEach(dirPath => {
            if (!lastDirStructure.has(dirPath)) {
              sendFileChangedEvent("addDir", dirPath);
            }
          });

          // Check for deleted directories
          lastDirStructure.forEach(dirPath => {
            if (!currentDirs.has(dirPath)) {
              sendFileChangedEvent("unlinkDir", dirPath);
            }
          });

          lastDirStructure = currentDirs;
        } catch (e) {
          // Ignore errors
        }
  ...
  try {
    ...
    if (!apiKey) {
      ...
      return {
        success: false,
        error:
          "Linear access token is not configured. Please set LINEAR_ACCESS_TOKEN as a system environment variable, or enter it in the app settings."
      };
    }
...
ipcMain.handle(
  "call-openai",
  async (_event: unknown, messages: Array<{ role: string; content: string }>, model: string = "gpt-4") => {
    try {
      ...
      if (!apiKey) {
        ...
        return {
          success: false,
          error:
            "OpenAI API key is not configured. Please set OPENAI_API_KEY as a system environment variable, or enter it in the app settings."
        };
      }
