...
export async function POST(request: Request) {
  try {
    ...
    if (!friend_id) {
      return NextResponse.json({ success: false, error: "Missing required field: friend_id" }, { status: 400 });
    }
    ...
    if (existingFriendship) {
      return NextResponse.json({ success: false, error: "Friendship already exists" }, { status: 409 });
    }
